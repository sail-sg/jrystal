...
def setup_qe():
  ...
  pp_dict = parse_upf('/home/aiops/litb/jrystal/pseudopotential/C.pbe-n-kjpaw_psl.1.0.0.UPF')
...
def setup_gpaw():
  ...
  # sys.path.insert(0, '/home/aiops/litb/M_p-align-claude/devs')
  # sys.path.insert(0, '/home/aiops/litb/jrystal/gpaw')
  from gpaw.setup_data import SetupData
  from gpaw_load import parse_paw_setup

  # Load GPAW setup file
  pp_data = parse_paw_setup('./C.PBE')
...
def calc(
  ...
  print("Delta_lq shape: ", Delta_lq.shape)
  with open("gpaw_pp_c.pkl", "wb") as f:
    pickle.dump({
      "r_g": jnp.array(r_g),
      "dr_g": jnp.array(dr_g),
      "Delta_pL": jnp.array(Delta_pL),
      'Qij': jnp.array(Qij),
      'proj': jnp.array(pt_jg)
    }, f)

  print("Arrays 'Qij' and 'proj' saved to 'gpaw_pp_c.pkl'")
