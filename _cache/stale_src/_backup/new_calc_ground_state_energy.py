...
def calc(config: JrystalConfigDict) -> GroundStateEnergyOutput:
  ...
  if config.k_point_parallel:
    if config.num_devices is not None:
      if config.num_devices > jax.device_count():
        logging.warning(
          f"num_devices indicated in config is {config.num_devices} which is "
          f"greater than the number of available devices {jax.device_count()}"
        )
  ...
  mesh = Mesh(
    np.array(jax.devices()[:num_devices]).reshape([1, -1]), ('s', 'k')
  )
  ...
  def total_energy(params_pw, params_occ, g_vec=g_vec):
    coeff = pw.coeff(params_pw, freq_mask, sharding)
...
if __name__ == "__main__":
  import jrystal
  config = jrystal.config.get_config("config.yaml")
  calc(config)
