...
def calc(
  ...
  num_devices = 1
  mesh = Mesh(np.array(jax.devices()[0:1]), ('s'))
  sharding = NamedSharding(mesh, P('s'))  # shard by the kpt dimension.
  ...
  #     params, k_vec.shape[0], config.occupation, config.spin_restricted
  #   )
  ...
  # )
  # ground_state_density_grid = jax.device_put(ground_state_density_grid, sharding)
  ...
  def eig_fn(
    ...
    hamil_matrix = hamiltonian._hamiltonian_matrix(
      coeff_k,
      crystal.positions,
      crystal.charges,
      ground_state_density_grid,
