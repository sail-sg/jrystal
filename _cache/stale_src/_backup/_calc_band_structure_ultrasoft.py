...
def calc(
  ...
  def hamiltonian_trace(
    params_pw_band, kpts, g_vector_grid, potential_nl, overlap_sqrt
  ):
    pw_coeff_band = pw.coeff(params_pw_band, freq_mask)
    ultrasoft_coeff_band = ultrasoft.ultrasoft_coefficients(
      pw_coeff_band, overlap_sqrt, crystal.vol
    )
    return ultrasoft._hamiltonian_trace(
      ultrasoft_coeff_band,
  ...
  def optimize_eigenvalues(kpts, beta_gk, params_pw_band, opt_state):
    ...
    carry, _ = jax.lax.scan(
      update_scan, (params_pw_band, opt_state, potential_nl_k, kpts[0:1], overlap_sqrt_k),
    ...
    def finetuning(carry, x):
      ...
      carry, _ = jax.lax.scan(
        update_scan, (params_pw_band, opt_state, potential_nl_k, kpts, overlap_sqrt_k),
    ...
    def eig_fn(param, kpts, potential_nl, g_vector_grid, overlap_sqrt):
      ...
      ultrasoft_coeff_i = ultrasoft.ultrasoft_coefficients(
        coeff_i, overlap_sqrt, crystal.vol
      )
  ...
  save_file = ''.join(crystal.symbol) + "_band_structure_ultrasoft.npy"
