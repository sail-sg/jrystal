...
def calc(config: JrystalConfigDict) -> GroundStateEnergyOutput:
  ...
  logging.info(f"Number of devices used: {util_devices / num_devices}.")
  ...
  def lagrangian(params_pw, params_occ, mu, temp, g_vec):
    free, (total, etro) = free_energy(params_pw, params_occ, temp, g_vec)
    return jnp.sum(
      free - mu * (jnp.sum(get_occupation(params_occ)) - num_electrons)
    ), (total, etro)
  ...
  params = {"pw": params_pw, "occ": params_occ, "mu": jnp.array([0.5])}
  ...
  with mesh:
    ...
    def update(params, opt_state, temp, g_vec):
      loss = lambda x: lagrangian(x["pw"], x["occ"], x["mu"], temp, g_vec)
    ...
    for i in iters:
      ...
      iters.set_description(
        ...
        f"Entropy: {entro:.4f}|T: {temp:.2E} | Occ: {jnp.sum(occ):.2f}"
      )
...
if __name__ == "__main__":
  ...
  config = jr.config.get_config()

  config.smearing = 0.01
  config.epoch = 10000
  config.verbose = True
  config.occupation = "simple"
  config.spin_restricted = True
  config.use_pseudopotential = False
  config.xc = "lda_x"
  config.k_grid_sizes = 2
  config.cutoff_energy = 40
