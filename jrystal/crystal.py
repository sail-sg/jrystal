...
from jrystal._src._const import ANGSTROM2BOHR
...
class Crystal:
  ...
  The crystal object represents the structure of a solid-state crystal, characterized by its periodic repetition. It encompasses all the details regarding the atoms and cells. It's a wrapper of the ASE(https://wiki.fysik.dtu.dk/ase/).
  ...
  def __init__(self,
    ...
    if symbols and (positions is not None) and (lattice_vectors is not None):
      self._ase_cell = ase.Atoms(symbols, positions, 
                                 cell=lattice_vectors, pbc=True)
    ...
    self.charges = jnp.array(self._ase_cell.get_global_number_of_atoms())
    ...
    self.natom = self._ase_cell.get_number_of_atoms()
