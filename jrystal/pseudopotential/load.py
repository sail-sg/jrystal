...
def parse_pp_info(pp_info_string: str) -> dict:
  ...
  valence_configuration_end_index = None
  for idx, line in enumerate(lines):
    line_lower = line.lower()
    if (
      valence_configuration_index is None and
      "valence configuration" in line_lower
    ):
      valence_configuration_index = idx
      continue
    if (
      valence_configuration_index is not None and
      "generation configuration" in line_lower
    ):
  ...
  if valence_configuration_index is None:
    warnings.warn(
      "pseudopotential does not find valence configuration.",
      stacklevel=2,
    )
  if valence_configuration_end_index is None:
  ...
  parsed_data = {
    ...
    'Valence configuration':
      parse_valence_configuration(
        [] if valence_configuration_index is None else
