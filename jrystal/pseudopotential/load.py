...
def parse_pp_info(pp_info_string: str) -> dict:
  ...
  parsed_data = {
    'Generated by': extract_value(lines[0]),
    'Author': extract_value(lines[1]),
    'Generation date': extract_value(lines[2]),
    'Pseudopotential type': extract_value(lines[3]),
    'Element': extract_value(lines[4]),
    'Functional': extract_value(lines[5]),
    'Suggested minimum cutoff for wavefunctions': extract_value(lines[6]),
    'Suggested minimum cutoff for charge density': extract_value(lines[7]),
    'Relativistic Calculation': lines[8].strip(),
    'Local Potential': extract_value(lines[9]),
    'Valence configuration': parse_valence_config(pp_info_string),
    'Generation configuration': parse_generation_config(pp_info_string)
  }

  return parsed_data


def parse_valence_config(text):
  lines = text.split('\n')
  valence_start = -1
  valence_end = -1
  ...
  for i, line in enumerate(lines):
    if "Valence configuration:" in line:
      valence_start = i
    elif valence_start != -1 and (
      "Generation configuration:" in line or "<" in line
    ):
      valence_end = i
      break
  ...
  if valence_start != -1:
    if valence_end == -1:
      valence_end = len(lines)
    valence_lines = lines[valence_start + 1:valence_end]

  valence_config = {}
  for line in valence_lines:
    line = line.strip()
    if not line or line.startswith("nl pn") or line.startswith("--"):
      continue
    parts = line.split()
    if len(parts) >= 7:
      ...
      valence_config[key] = {
        ...
        "occ": float(parts[3]),
        "Rcut": float(parts[4]),
        "Rcut US": float(parts[5]),
        "E pseu": float(parts[6]),
      }

  return valence_config


def parse_generation_config(text):
  lines = text.split('\n')
  generation_start = -1
  generation_end = -1
  ...
  for i, line in enumerate(lines):
    if "Generation configuration:" in line:
      generation_start = i
    elif generation_start != -1 and ("<" in line or "</PP_INFO>" in line):
      generation_end = i
      break
  ...
  if generation_start != -1:
    if generation_end == -1:
      generation_end = len(lines)
    generation_lines = lines[generation_start + 1:generation_end]
        ...
        augmentation_data['PP_QIJ'].append(
          {
            ...
            'angular_momentum': qij.get('angular_momentum', 0),
