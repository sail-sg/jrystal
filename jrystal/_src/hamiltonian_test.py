...
from jrystal._src.hamiltonian import (
  _hamiltonian_matrix_basis, hamiltonian_matrix, hamiltonian_matrix_trace,
  _hamiltonian_matrix, hamiltonian_matrix_basis_diag
)
...
class _TestHamiltonian(parameterized.TestCase):
  ...
  def setUp(self):
    ...
    diamond_file_path = jr.get_pkg_path() + "/geometry/diamond.xyz"
  ...
  def test_hamiltonian_matrix_basis_diag(self):
    h1 = hamiltonian_matrix_basis_diag(
      self.coeff,
      self.freq_mask,
      self.crystal.positions,
      self.crystal.charges,
    ...
    h2 = _hamiltonian_matrix_basis(
      ...
      self.crystal.vol,
      kohn_sham=False
    )  # [kpt basis basis]  basis = 80?

    print(h2.shape)
    h1 = jr.utils.squeeze_coefficient(h1, self.freq_mask)  # [s k g b]
    h2 = jax.vmap(jnp.diag)(h2)
