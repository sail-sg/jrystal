...
def get_xc_functional(xc: str = "gga_x_pbe", polarized: bool = False):
  ...
  try:
    ...
    raise ImportError(f"Could not import {polarization} from {module_path}: {e}")
...
def _gga(xc_type: str, rho_r: Float[Array, "s x y z"], sigma_r: Float[Array, "s x y z"]):
  polarized, is_exchange, p, f, grid_shape, rho_r_flat = _parse_xc_type(xc_type, rho_r)
  sigma_r_flat = sigma_r.reshape(3 if polarized else 1, -1)
  if polarized and is_exchange:  # NOTE: exact scaling relation
    f_up = jax.vmap(f, (None, 0, 0), 0)(p, 2 * rho_r_flat[0], 4 * sigma_r_flat[0])
    # NOTE: sigma_r_flat[1] is the cross term
    f_dn = jax.vmap(f, (None, 0, 0), 0)(p, 2 * rho_r_flat[1], 4 * sigma_r_flat[2])
    f_val = 0.5 * (f_up + f_dn)
  elif polarized:  # and is correlation
...
def vxc_gga_recp(
  ...
  if polarized:
    ...
    dexc_dsigma_flat = jax.vmap(jax.grad(exc, argnums=1))(rho_r_flat[0], sigma_r_flat[0])
...
def xc_density(
  ...
  if "gga" in xc_type:
    ...
    exc_fn = lambda density: sum([_lda(xc_type_, density) for xc_type_ in xc_type.split("+")])
