...
def _ifftn_jvp(primals, tangents):
  return _ifftn_impl(*primals), _ifftn_impl(*tangents)


def _ifftn_tranpose_rule(cotangent, x):
  return _ifftn_impl(cotangent),


def _fftn_jvp(primals, tangents):
  return _fftn_impl(*primals), _fftn_impl(*tangents)


def _fftn_tranpose_rule(cotangent, x):
  return _fftn_impl(cotangent),
...
jax._src.interpreters.batching.primitive_batchers[_ifftn_p] = _ifftn_batching_rule
