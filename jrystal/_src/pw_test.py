...
class _TestModules(parameterized.TestCase):
  ...
  def test_wave(self):
    pw_param = jr.pw.param_init(self.key, self.num_bands, 1, self.freq_mask)
    coeff = jr.pw.coeff(pw_param, self.freq_mask)
  ...
  def test_nabla_n_grid(self):
    ...
    nabla_density_r1 = jr._src.pw.nabla_density_grid(
